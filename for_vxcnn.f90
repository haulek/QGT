...
subroutine calc_minm_MT(mmat, ql, nstart,nend,mstart,mend, alfa,beta,gama,alfp,betp,gamp, s3r, pos, mult, nmix, big_l, nLO_at, ncore, cgcoef, lmax, loctmatsize, nbmax, mbmax, nat, ntnt, nLOmax, ndf, n_mix, nhow_many_fnc, max_nmix, lomax, nbmax_g,mbmax_g,ntnt_g,ndf_g)
  ...
  !                               mmat(ie1,ie2,im) = < u^{product}_{im,lb} | psi^*_{ie2,k-q} psi_{ie1,k} > e^{-iq.r_atom}
  ...
  real(8), parameter  :: pi = 3.14159265358979d0
  logical   :: PRINT
  integer   :: iat, lb, mb, idf, ie1, ie2, ieq, l1_, l2_
  real(8)   :: angint
  ...
  if (PRINT) then
     write(6,*) 'calcminm'
     write(6,*) 'nLOmax=', nLOmax
     !write(6,*) 'ql=', ql
     !write(6,*) 'nstart=', nstart, 'nend=', nend, 'mstart=', mstart, 'mend=', mend
     !write(6,*) 'shape(alfa)=', nbmax, ntnt, ndf
     !write(6,*) 'shape(gama)=', nbmax_g, nLOmax, ntnt_g, ndf_g
     !write(6,*) 'shape(alfp)=', mbmax
     !write(6,*) 'pos=', pos
     !write(6,*) 'mult=', mult
     !write(6,*) 'nmix=', nmix
     !write(6,*) 'big_l=', big_l
     !write(6,*) 'nLOmax=', nLOmax
     !write(6,*) 'loctmatsize=', loctmatsize
     !write(6,*) 'nLO_at=', nLO_at
     !write(6,*) 'shape(big_l)=', max_nmix,nat
     !write(6,*) 'shape(s3)=', n_mix,nhow_many_fnc,nhow_many_fnc,nat
     !write(6,*) 'lomax=', lomax
  endif
  cind=0
  ...
  do iat = 1, nat               !! Loop over atoms
     ...
     do l2=0,lomax
        do m2=-l2,l2
           ...
           do ilo2 = 1,nLO_at(l2+1,iat)
              ncl2 = ncl2 + 1
              cind(ilo2,lm2) = ncl2   ! just index to successive localized orbital, so that we can use ilo,lm
           enddo
        enddo
     enddo
     if (ncl2 > 0) then
     ...
     do ieq = 1, mult(iat)      !! Loop over equivalent atoms
        idf = idf + 1
        ! arranging alfp,betp,gamp so that we have only needed bands, and orbitals corresponding to this atom
        ! in talfp, tbetp, tgamp
        do ie2 = 1,mend-mstart
           do lm2=1,(lmax+1)**2
              talfp(lm2,ie2) = alfp(ie2+mstart,lm2,idf)
              tbetp(lm2,ie2) = betp(ie2+mstart,lm2,idf)
        ...
        !
        phs = exp( -dot_product( ql, pos(:,idf) )*two_pi_i )  ! e^{-i*q_cart_semi_cartesion*pos_idf_semi_cartesian}
        !
        do irm = 1, nmix(iat)   !!  Loop over mixed functions:
           lb = big_l(irm,iat)
           do mb = -lb,lb
              ...
              do l1 = 0, lmax              ! index of the single-particle function u_{lm}
                 ...
                 lb1 = la1 + lmax + 1      ! why extra 1 I do not understand!!!
                 l1_ = min(l1,lomax+1)
                 lc10 = ncore(iat) + 2*(lmax+1) + sum(nLO_at(:l1_,iat))  ! where local orbitals start
                 do l2 = l2min,l2max           !! loop over l2:
                    la2 = l2 + ncore(iat) + 1  
                    lb2 = la2 + lmax + 1
                    l2_ = min(l2,lomax+1)
                    lc20 = ncore(iat) + 2*(lmax+1) + sum(nLO_at(:l2_,iat)) ! where local orbitals start
                    do m1 = -l1,l1             
                       lm1 = l1*l1 + l1 + m1 + 1   ! index for Y_{l1,m1}
                       m2 = m1 - mb                ! This comes from matrix element <Y_{l1m1}|Y_{l2,m2}|Y_{lb,mb}>, which requires m1==mb+m2
                       if ( abs(m2) .gt. l2) cycle 
                       ...
                       phs_angint = phs*angint
                       !
                       aa = phs_angint * s3r(irm,la1,la2,iat) ! aa = <u_{irm,lb}|    u_{a1,l1}    u_{a2,l2}> <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                       ab = phs_angint * s3r(irm,lb1,la2,iat) ! ab = <u_{irm,lb}| udot_{a1,l1}    u_{a2,l2}> <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                       ba = phs_angint * s3r(irm,la1,lb2,iat) ! ba = <u_{irm,lb}|    u_{a1,l1} udot_{a2,l2}> <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                       bb = phs_angint * s3r(irm,lb1,lb2,iat) ! bb = <u_{irm,lb}| udot_{a1,l1} udot_{a2,l2}> <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                       if (l1<=lomax) then
                          do ilo1 = 1,nLO_at(l1+1,iat) 
                             lc1 = lc10 + ilo1
                             ac(ilo1) = phs_angint * s3r(irm,lc1,la2,iat) ! ac = <u_{irm,lb}| u_{lo,a1,l1}    u_{a2,l2} > <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                             bc(ilo1) = phs_angint * s3r(irm,lc1,lb2,iat) ! bc = <u_{irm,lb}| u_{lo,a1,l1} udot_{a2,l2}|> <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                          enddo
                       endif
                       if (l2<=lomax) then
                          do ilo2 = 1,nLO_at(l2+1,iat)
                             lc2 = lc20 + ilo2
                             ca(ilo2) = phs_angint * s3r(irm,la1,lc2,iat) ! ca = < u_{irm,lb} |    u_{a1,l1} ulo_{a2,l2} > <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                             cb(ilo2) = phs_angint * s3r(irm,lb1,lc2,iat) ! cb = < u_{irm,lb} | udot_{a1,l1} ulo_{a2,l2} > <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                             if (l1<=lomax) then
                                do ilo1 = 1,nLO_at(l1+1,iat)
                                   lc1 = lc10 + ilo1
                                   cc(ilo1,ilo2) = phs_angint * s3r(irm,lc1,lc2,iat) ! cc = < u_{irm,lb} | ulo_{a1,l1}) ulo_{a2,l2} > <Y_{lb,mb}|Y_{l1,m1}Y*_{l2,m2}>*e^{-iq.r_atom}
                                enddo
                             endif
                          enddo
                       endif
                       ! na = < u_{irm,lb} |    u_{a2,l2} psi_{ie1} >e^{-iq.r_atom} = < u_{irm,lb} | (alfa u_{a1,l1} + beta udot_{a1,l1} + gama ulo_{a1,l1}) *    u_{a2,l2} >e^{-iq.r_atom}
                       ! nb = < u_{irm,lb} | udot_{a2,l2} psi_{ie1} >e^{-iq.r_atom} = < u_{irm,lb} | (alfa u_{a1,l1} + beta udot_{a1,l1} + gama ulo_{a1,l1}) * udot_{a2,l2} >e^{-iq.r_atom}
                       ! nc = < u_{irm,lb} |  ulo_{a2,l2} psi_{ie1} >e^{-iq.r_atom} = < u_{irm,lb} | (alfa u_{a1,l1} + beta udot_{a1,l1} + gama ulo_{a1,l1}) *  ulo_{a2,l2} >e^{-iq.r_atom}
                       do ie1=1,nend-nstart                        !! Loop over eigenfunctions at k
                          ie1p = ie1+nstart
                          ...
                          if (l1<=lomax) then
                             do ilo1 = 1,nLO_at(l1+1,iat)
                                na(ie1,lm2) = na(ie1,lm2) + ac(ilo1) * gama(ie1p,ilo1,lm1,idf) ! na += <u_{irm,lb}| u_{lo,a1,l1}    u_{a2,l2} > * gama(ie1,ilo,l1m1)<Y|Y|Y>e^{-iq.r_atom}
                                nb(ie1,lm2) = nb(ie1,lm2) + bc(ilo1) * gama(ie1p,ilo1,lm1,idf) ! nb += <u_{irm,lb}| u_{lo,a1,l1} udot_{a2,l2}|> * gama(ie1,ilo,l1m1)<Y|Y|Y>e^{-iq.r_atom}
                             enddo
                          endif
                          if (l2<=lomax) then
                             do ilo2 = 1,nLO_at(l2+1,iat)
              ...
              !! sm = < u_{irm,lb} | psi^*_{ie2} psi_{ie1} >e^{-iq.r_atom}
              call zgemm('n','n', nend-nstart,mend-mstart,(lmax+1)**2, cone, na,nend-nstart, talfp,(lmax+1)**2, czero, mmt2,nend-nstart) ! mmt2  = matmul(na[ie1,lm], talfp[lm,ie2])
              call zgemm('n','n', nend-nstart,mend-mstart,(lmax+1)**2, cone, nb,nend-nstart, tbetp,(lmax+1)**2, cone,  mmt2,nend-nstart) ! mmt2 += matmul(nb[ie1,lm], tbetp[lm,ie2])
              if (ncl2 > 0) then
                 call zgemm('t','n', nend-nstart,mend-mstart,ncl2, cone, nc,ncl2, tgamp,ncl2, cone,  mmt2,nend-nstart)  ! mmt2 += matmul(nc[ie1,lm], tgamp[lm,ie2])
              endif
              mmat(:,:,im) = mmt2(:,:)
              if (.False.) then
                 do ie1=1,nend-nstart
...
subroutine calc_minm_IS(mmat, nstart,nend,mstart,mend, Aeigk,Aeigq, iumklap, mpwipw,  nvi,nvj, indgk_i, indgk_j, indgq_inverse, gindex, i_g0, Vol, ngq, ngq_barc, ng, n1, n2, n3, ngi, ngj, inv_size, nbmax_i, nbmax_j)
  ...
  do iGp = 1,ngq                          ! over G in the product-interstitial basis
     ...
     if (PRINT) then
        WRITE(117,*) 'ngq=', ngq, 'ngq_barc=', ngq_barc, 'n1=', n1, 'n2=', n2, 'n3=', n3, 'inv_size=', inv_size
        do ivi=1,nvi
           do ivj=1,nvj
              iGv = gindex(indgk_i(ivi)+1,:) - gindex(indgk_j(ivj)+1,:) + iumklap(:)  ! G_ki - G_kj + umklap : reciprocal vectors from vector file
              idg = 0
              ...
              write(117,'(I3,1x,I3,1x,I3,3x,2F12.8,3x,I5,3x,I5)') iGp, ivi, ivj, tmat(ivj,ivi), iKpw(ivi,ivj), idg
           enddo
        enddo
        close(117)
     endif
     ...
     !
     !    A^*(k-q,G_kj,ie2) * Int[e^{i(G_ki - G_kj + umklap)*r}]_{interstitial} * A.T(k,G_ki,ie1)
     !
     ...
     call zgemm('n','n', mdim,ndim,nvj, cone, Aeigq(mstart+1:mend,:nvj),mdim, tmat2,nvj, czero, mnn,mdim)  ! why not conjugated: because Aeigq is supposed to be conjugated already in python
     do ie1 = 1,nend-nstart
        do ie2 = 1,mend-mstart
           mmat(ie1,ie2,iGp) = mnn(ie2,ie1)*sqvi ! note, we are transposing here!
        enddo
     enddo
...
!subroutine Compute_M_P_M_notetra(eps, epsw1, minm2, enk, enq, iq, No, Neactual, coef, omega, pm, matsize, NoNe, Noactual, Ne, nom)
!  IMPLICIT NONE
!  COMPLEX*16, intent(inout) :: eps(nom,matsize,matsize)
!  COMPLEX*16, intent(inout) :: epsw1(matsize,nom)
!  COMPLEX*16, intent(in) :: minm2(NoNe,matsize)
!  REAL*8, intent(in)  :: enk(Noactual)
!  REAL*8, intent(in)  :: enq(Ne)
!  REAL*8, intent(in)  :: omega(nom)
!  COMPLEX*16, intent(in) :: pm(NoNe)
!  INTEGER, intent(in) :: No, Neactual, iq
!  INTEGER, intent(in) :: Ne, Noactual, NoNe, matsize, nom
!  REAL*8, intent(in) :: coef
!  !
!  INTEGER :: io, ie, ioie, ia, ib, iw
!  REAL*8 :: kcw2(nom)
!  REAL*8 :: dE
!  COMPLEX*16 :: mm2
!  COMPLEX*16 :: kcw3(nom)
!  do io=1,Noactual
!     write(6,*) 'io=', io, 'ioie=', ioie
!     do ie=1+Ne-Neactual,Ne
!        ioie = (io-1)*Ne + ie
!        dE = enq(ie) - enk(io)
!        do iw=1,nom
!           kcw2(iw) = -2*coef*dE/( omega(iw)*omega(iw) + dE*dE)
!        enddo
!        do ia=1,matsize
!           do ib=1,matsize
!              mm2 = minm2(ioie,ia)*conjg(minm2(ioie,ib))
!              do iw=1,nom
!                 eps(iw,ia,ib) = eps(iw,ia,ib) + mm2 * kcw2(iw)
!              enddo
!           enddo
!        enddo
!     enddo
...
!           dE = enq(ie) - enk(io)
!           do iw=1,nom
!              kcw3(iw) = -2*pm(ioie)*coef*dE/( omega(iw)*omega(iw) + dE*dE)
!           enddo
!           do ia=1,matsize
!              do iw=1,nom
!                 epsw1(ia,iw) = epsw1(ia,iw) + minm2(ioie,ia)*kcw3(iw)
!              enddo
!           enddo
!        enddo
!     enddo
!  endif
!end subroutine Compute_M_P_M_notetra

subroutine Compute_M_P_M_notetra2_orig(eps, epsw1, kcw, minm2, enk, enq, omega, pm, iq, Noactual, Neactual, coef, ankp, matsize, NoNe, No, Ne, nom,No_,Ne_,NoNe_)
  IMPLICIT NONE
  COMPLEX*16, intent(inout) :: eps(matsize,matsize,nom)
  COMPLEX*16, intent(inout) :: epsw1(matsize,nom)
  COMPLEX*16, intent(in) :: minm2(NoNe,matsize)
  REAL*8, intent(in)  :: enk(No)  ! enk(ncg+ks.ibmin_tetra+1)
  REAL*8, intent(in)  :: enq(Ne)        ! enq(ks.nbmaxpol-ncmb)
  REAL*8, intent(in)  :: omega(nom)
  REAL*8, intent(in)  :: kcw(No_,Ne_,nom) ! No=20,Ne=205,nom=32,Noactial=20,Ne=205
  COMPLEX*16, intent(in) :: pm(NoNe_)
  INTEGER, intent(in) :: Noactual, Neactual, iq, ankp
  INTEGER, intent(in) :: Ne, No, NoNe, matsize, nom, No_,Ne_,NoNe_
  REAL*8, intent(in) :: coef
  !
  INTEGER :: io, ie, ioie, ia, ib, iw
  REAL*8 :: kcw2
  REAL*8 :: dE
  COMPLEX*16, allocatable :: cminm2(:,:), mm2(:)
  complex*16 :: czero, cone
  czero = cmplx(0.d0,0.d0,8)
  cone  = cmplx(1.d0,0.d0,8)
  allocate(cminm2(NoNe,matsize), mm2(matsize))
  cminm2=0
  if ((No_ .ne. No-Noactual) .or. (Ne_ .ne. Ne-Neactual)) then
     WRITE(6,*) 'ERROR: No_=', No_, 'expecting', No-Noactual, 'Ne_=', Ne_, 'expecting', Ne-Neactual
  endif
  do iw=1,nom
     do io=1,Noactual ! ncg+ks.ibmin_tetra+1
        do ie=1,Ne    ! ks.nbmaxpol-ncbm
           ioie = (io-1)*Ne + ie
           dE = enq(ie) - enk(io)
           kcw2 = -(2.0/ankp)*dE/( omega(iw)*omega(iw) + dE*dE)
           cminm2(ioie,:) = coef*kcw2*conjg(minm2(ioie,:))
        end do
     enddo
     do io=Noactual+1,No     ! ncg+ks.ibmin_tetra+1,ncg+nvbm
        do ie=1,Ne-Neactual  ! ks.ibmax_tetra-ncbm
           ! Only in this range we actually use tetrahedrons. The low energy part
           ioie = (io-1)*Ne + ie
           !dE = enq(ie) - enk(io)
           !kcw2 = -(2.0/ankp)*dE/( omega(iw)*omega(iw) + dE*dE)
           kcw2 = kcw(io-Noactual,ie,iw)
           cminm2(ioie,:) = coef*kcw2*conjg(minm2(ioie,:))
        enddo
        do ie=1+Ne-Neactual,Ne
           ioie = (io-1)*Ne + ie
     ...
     enddo
     if (iq==0) then
        mm2 = matmul(conjg(pm),cminm2)
        epsw1(:,iw) = epsw1(:,iw) + conjg(mm2)
     endif
     ! eps += minm2.T * (coef*kcw) * cminm2
     ! eps(alpa,beta) += minm2.T(alpa,ioie) * (coef * kcw(ioie)) * conjg(minm2(ioie,beta))
     call zgemm('T', 'N', matsize,matsize,NoNe, cone, minm2, NoNe, cminm2, NoNe, cone, eps(:,:,iw), matsize)
  enddo
  deallocate(cminm2, mm2)
end subroutine Compute_M_P_M_notetra2_orig


subroutine Compute_M_P_M_notetra2(eps, epsw1, kcw, minm2, enk, enq, omega, pm, iq, Noactual, Neactual, coef, ankp, matsize, NoNe, No, Ne, nom,No_,Ne_,NoNe_)
  IMPLICIT NONE
  COMPLEX*16, intent(inout) :: eps(matsize,matsize,nom)
  COMPLEX*16, intent(inout) :: epsw1(matsize,nom)
  COMPLEX*16, intent(in) :: minm2(NoNe,matsize)
  ...
  czero = cmplx(0.d0,0.d0,8)
  cone  = cmplx(1.d0,0.d0,8)
  allocate(cminm2(matsize,NoNe), mm2(matsize))
  !cminm2= transpose(minm2)
  !cminm2 = conjg(cminm2)
  if ((No_ .ne. No-Noactual) .or. (Ne_ .ne. Ne-Neactual)) then
     WRITE(6,*) 'ERROR: No_=', No_, 'expecting', No-Noactual, 'Ne_=', Ne_, 'expecting', Ne-Neactual
  endif
  do iw=1,nom
     cminm2 = conjg(transpose(minm2))
     do io=1,Noactual ! ncg+ks.ibmin_tetra+1
        do ie=1,Ne    ! ks.nbmaxpol-ncbm
           ioie = (io-1)*Ne + ie
           dE = enq(ie) - enk(io)
           kcw2 = -(2.0/ankp)*dE/( omega(iw)*omega(iw) + dE*dE)
           cminm2(:,ioie) = cminm2(:,ioie)*coef*kcw2
        end do
     enddo
     do io=Noactual+1,No     ! ncg+ks.ibmin_tetra+1,ncg+nvbm
        do ie=1,Ne-Neactual  ! ks.ibmax_tetra-ncbm
     ...
     enddo
     if (iq==0) then
        mm2 = matmul(cminm2,conjg(pm))
        epsw1(:,iw) = epsw1(:,iw) + conjg(mm2)
     endif
     ! eps += minm2.T * (coef*kcw) * cminm2.T
     ! eps(alpa,beta) += minm2.T(alpa,ioie) * (coef * kcw(ioie)) * conjg(minm2(ioie,beta))
     call zgemm('T', 'T', matsize,matsize,NoNe, cone, minm2, NoNe, cminm2, matsize, cone, eps(:,:,iw), matsize)
  enddo
  deallocate(cminm2, mm2)
end subroutine Compute_M_P_M_notetra2

subroutine Compute_M_P_M_notetra2_2(eps, epsw1, kcw, minm2, enk, enq, omega, pm, iq, Noactual, Neactual, coef, ankp, matsize, NoNe, No, Ne, nom,No_,Ne_,NoNe_)
  IMPLICIT NONE
  COMPLEX*16, intent(inout) :: eps(matsize,matsize,nom)
  COMPLEX*16, intent(inout) :: epsw1(matsize,nom)
  COMPLEX*16, intent(in) :: minm2(NoNe,matsize)
  ...
  if ((No_ .ne. No) .or. (Ne_ .ne. Ne)) then
     WRITE(6,*) 'ERROR: No_=', No_, 'expecting', No, 'Ne_=', Ne_, 'expecting', Ne
  endif
  !write(6,*) 'No=', No, 'Ne=', Ne, 'No_=', No_, 'Ne_=', Ne_

  ! cminm2 = minm2.H(al,ioie)*coef*kcw(ioie)
  ! minm2.T * cminm2.T = minm2(al,ioie)*minm2.*(ioie,beta)
  
  ! coef*minm2.T*kcw[:No,:Ne]*minm2^*
  ! minm2.T(al,ioie) coef*kcw*minm2^*(ioie,beta)
  do iw=1,nom
     cminm2 = conjg(transpose(minm2))
     do io=1,Noactual ! ncg+ks.ibmin_tetra+1
        do ie=1,Ne    ! ks.nbmaxpol-ncbm
           ...
           kcw2 = kcw(io,ie,iw)
           cminm2(:,ioie) = cminm2(:,ioie)*coef*kcw2
           !write(6,'(I4,1x,I4,1x,F15.8,2x,4I4)') ioie, NoNe, kcw2, shape(cminm2), io, ie
        end do
     enddo
     do io=Noactual+1,No     ! ncg+ks.ibmin_tetra+1,ncg+nvbm
        do ie=1,Ne-Neactual  ! ks.ibmax_tetra-ncbm
  ...
  deallocate(cminm2, mm2)
end subroutine Compute_M_P_M_notetra2_2
