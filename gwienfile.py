...
class RadialFunctions:
    def __init__(self, in1, strc, Elapw, Elo, Vr, nspin, fout):
        ...
        for isp in range(nspin):
            for iat in range(strc.nat):
                print("-----  Calc radfunc for atom ",iat, file=fout)
                print("-- (L)APW states --", file=fout)
                print((' '*10+'POTENTIAL PARAMETERS FOR JATOM=%-3d name=%-10s') % (iat,strc.aname[iat]), file=fout)
                print(' '*11+'L'+(' '*7)+'U(R)'+(' '*11)+"U'(R)",(' '*9)+'DU/DE'+(' '*10)+"DU'/DE"+(' '*9)+"NORM-U'", file=fout)
                ...
                for l in range(in1.nt):
                    ...
                    a *= trx
                    b *= trx
                    #print('l=%2d, EI=%20.14f, uv=%20.14f, duv=%20.14f, ovlp=%25.14f' % (l,eh,uv,duv,ovlp))
                    ...
                    ac *= trx
                    bc *= trx
                    #print('l=%2d, EI=%20.14f, uv=%20.14f, duv=%20.14f, ovlp=%25.14f' % (l,eh-dele,uvc,duvc,ovlp))
                    ...
                    ae *= trx
                    be *= trx
                    #print('l=%2d, EI=%20.14f, uv=%20.14f, duv=%20.14f, ovlp=%25.14f' % (l,eh+dele,uve,duve,ovlp))
                    ...
                    if( cross > 0.05): print('For l='+str(l)+' correction='+str(-cross)+' overlap='+str(ovlp), file=fout)
                    ...
                    #print('l=%2d, EI=%20.14f, P=%20.14f, dP=%20.14f, PE=%20.14f, dPE=%20.14f' % (l,eh,uv,duv, uve, duve))
                    ...
                    print((' '*10+'%2d '+'%14.6f '*5+' '*5+'%2d '*3) % (l,self.umt[isp,iat,l,0],self.umt[isp,iat,l,2],self.umt[isp,iat,l,1],self.umt[isp,iat,l,3],self.umt[isp,iat,l,4],nodel,nodes,nodeu), file=fout)
        ...
        for isp in range(nspin):
            for iat in range(strc.nat):
                print((' '*10+'LOCAL ORBITAL POTENTIAL PARAMETERS FOR JATOM=%-3d name=%-10s') % (iat,strc.aname[iat]), file=fout)
                print((' '*11)+'L'+(' '*7)+'U(R)'+(' '*10)+"U'(R)"+(' '*9)+'<u|u_LO>'+(' '*6)+'<u_dot|u_LO>'+' num. nodes'+ ' Elo', file=fout)
                npt = strc.nrpt[iat]
                dh  = log(strc.rmt[iat]/strc.r0[iat])/(npt - 1)      # logarithmic step for the radial mesh
                for l in range(lomaxp1):
                    for ilo in in1.nLO_at_ind[iat][l]:
                        ...
                        self.umtlo[isp,iat,l,ilo,0] = uv      # plo  = u_{lo}(R_MT)
                        self.umtlo[isp,iat,l,ilo,1] = duv     # dplo = udot_{lo}(R_MT)
                        self.umtlo[isp,iat,l,ilo,2] = pi12lo  # pi12lo= <u_{lo}|u_l>
                        self.umtlo[isp,iat,l,ilo,3] = pe12lo  # pe12lo= <u_{lo}|udot_l>
                        #   Store the radial wave function :
                        self.ulo [isp,iat,l,ilo,:npt] = a[:]  # local orbital u_lo(r)
                        self.uslo[isp,iat,l,ilo,:npt] = b[:]  # relativistic part of local orbital u_lo(r)
                        
                        print(((' '*10)+'%2d '+('%14.6f'*4)+(' '*5)+'  %2d   '+' %10.6f') % (l,uv,duv,pi12lo,pe12lo,nodes,el*2), file=fout)
                for l in range(lomaxp1):
                    if len(in1.nLO_at_ind[iat][l])>0:
                        print(' '*10+'LOCAL ORBITALS OVERLAPS:', file=fout)
                        print(' '*10+'jlo   klo      <u|u>           E[jlo]          E[klo]', file=fout)
                    for jlo in in1.nLO_at_ind[iat][l]:
                        for klo in in1.nLO_at_ind[iat][l]:
                            ...
                            print(((' '*7)+('%5d '*2)+('%15.6f '*3)) % (jlo,klo,pilolo,Elo[isp,iat,l,jlo],Elo[isp,iat,l,klo]), file=fout)
            ...
            for iat  in range(nat):
                ...
                for l in range(lomaxp1):
                    ...
                    if in1.lapw[l,iat]==0:  # APW+lo, hence ony u and dotu are used
                        ...
                        print('lo coefficient: iat=%-2d l=%-2d ilo=%-1d lapw=%1d a=%-12.7f b=%-12.7f c=%-12.7f' % (iat,l,ilo,in1.lapw[l,iat],self.abcelo[isp,iat,l,ilo,0],self.abcelo[isp,iat,l,ilo,1],self.abcelo[isp,iat,l,ilo,2]), file=fout)
                    for ilo in in1.nLO_at_ind[iat][l]:
...
class CoreStates:
    def __init__(self, case, strc, nspin, fout):
        ...
        print('read core states occupuation information!', file=fout)
        ...
        for iat in range(strc.nat):
            ...
            print('Number of core states at '+str(iat)+'-th atom:', ncore[iat], file=fout)
        line = next(fin)
        ncoremax = max(ncore)
        ...
        if ncoremax != 0:
            ...
            for iat in range(strc.nat):
                ...
                if (t_norb != len(self.occ_inc[iat])):
                    print('WARNING : reading '+case+'.core : norb read on top=', len(self.occ_inc[iat]),  'while t_norb=', t_norb, file=fout)
                print('N t  nm oc kp l Ene[Hartree]', file=fout)
                for iorb in range(t_norb):
                    ...
                    if self.occ_inc[iat][iorb] < 1e-2:
                        print(' the core state ', t_nm, ' is frozen', file=fout)
                        continue
                    kappa = self.l2kappa[t_symb]
                    lc = int( abs(kappa)-(1-sign(kappa))/2 )
                    ...
                    Ry2H = 0.5  
                    print(t_n,t_symb,t_nm, self.occ_inc[iat][iorb], '%2d %1d' % (kappa,lc), t_ec*Ry2H, file=fout)
            if nspin==2:
                isp=1
                for iat in range(strc.nat):
                    line = next(fin)
            ...
            print('l_core_max=', l_core_max, 'nclm_max=', nclm_max, file=fout)
            ...
            for  isp in range(nspin):
                for iat in range(strc.nat):
                    ...
                    npt = strc.nrpt[iat]
                    norb = int(next(fin).split()[0])
                    print(' total number of core wave functions from '+case+'.core = ', norb, file=fout)
                    for iorb in range(norb):
                        line = next(fin)
                        nm = line[17:20]
                        print(' read orbital '+nm+' with occ=',self.occ_inc[iat][iorb], file=fout)
                        ulc = zeros((2,npt))
                        for idot in [0,1]:
                            ipt = 0
                            while(fin):
                                ...
                                if ipt >= npt: break
                        if self.occ_inc[iat][iorb] < 1e-2:
                            print("  exclude core state ", nm, file=fout) 
                        else:
                            ic = len(self.ul_core[isp][iat])
                            lc = self.l_core[iat][ic]
                            t_n, t_nm, kappa, max_occ = n_sym_kap_ocm[iat][ic]
                            if (nm != t_nm):
                                print('WARNING reading '+case+'.core at iat='+str(iat)+' iorb='+str(iorb)+' nm='+nm+' t_nm='+t_nm, file=fout)
                            ...
                            print('    and the wave function for state ', nm,'. It is renormalized with coefficient %10.7f to account for proper degeneracy.' % (norm**2,), file=fout)
            fin.close()
        else:
            nclm_max = 0
        ...
        print(file=fout)
        print(' '*10+'Total number of electrons:      %8.4f' % (ntot,), file=fout)
        print(' '*10+'Number of valence electrons:    %8.4f' % (nval,), file=fout)
        print(' '*10+'Number of core electrons:       %8.4f' % (ne_core,), file=fout)
        print(' '*10+'Number of frozen core electrons:%8.4f' % (nelfc,), file=fout)
        ...
        print(' '*10+'Total Number of Core states:     ', sum(self.nclm), len(self.corind), file=fout)
        self.corind = array(self.corind, dtype=int)
        if True:
            print('corrind index=', file=fout)
            for ic in range(len(self.corind)):
                print('iat=%3d idf=%3d ic=%3d lc=%3d mc=%3d' % tuple(self.corind[ic]), ' with weight=sqrt(%10.7f)' % (remember_weight[ic],), file=fout)
