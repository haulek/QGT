from numpy import array,sqrt,pi,dot,ones,zeros,amax
...
class PlaneWaves:
    ...
    def __init__(self, hsrws, kmr, pwm, case, strc, in1, latgen, kqm, debug, fout):
        ...
        # WARNING: This is a workaround so that we do not need vxc file
        (heads, all_Gs, all_As, all_Ek) = w2k.Read_vector_file(case, strc.nat, fout)
        kxcmax = max([amax(abs(Gs)) for Gs in all_Gs])
            ...
            for ii,ki in enumerate(itertools.product(list(range(-ng[0],ng[0]+1)),list(range(-ng[1],ng[1]+1)),list(range(-ng[2],ng[2]+1)))):
                ...
                if kk < gmax :
                    ...
                    if ortho:
                        ...
                        gindex.append( list(map(int, (kc/latgen.pia).round())) )
                    else:
