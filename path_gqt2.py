...
def Find_nbs_nbei(ik, Ebnd, pairs_ik, nbs, nbe, fout):
    ...
    while ibe<len(Ene):
        if abs(Ene[ibe]-Ene[ibe-1])<1e-6:
            ...
            break
    #if (ibs!=nbs):
    #    print('At ik=', ik, 'nbs=', nbs, 'decreased to', ibs, file=fout)
    #if (ibe!=nbe):
    #    print('At ik=', ik, 'nbe=', nbe, 'increased to', ibe, file=fout)
    ...
    #if corrected:
    #    if (ibs!=nbs):
    #        print('Corr ik=', ik, 'nbs=', nbs, 'increased to', ibs, file=fout)
    #    if (ibe!=nbe):
    #        print('Corr ik=', ik, 'nbe=', nbe, 'increased to', ibe, file=fout)
    return (ibs,ibe)
...
def Find_nbs_nbe(Ebnd, nbs, nbe, pairs, fout):
    nbs_nbe=[]
    for iik,ik in enumerate(pairs):
        nbs_nbe.append( Find_nbs_nbei(ik, Ebnd, pairs[ik], nbs, nbe, fout) )
    return nbs_nbe
...
def Compute_mmn(nbs,nbe):
    ...
    k0shift=[0,0,0]
    if mrank==master: print('kdivs=', nkdivs)

    kqm = KQmesh(nkdivs, k0shift, strc, latgen, fout)
    pw = PlaneWaves(ks.hsrws, kmr, pwm, case, strc, in1, latgen, kqm, False, fout)

    nsp, nkp, band_max = shape(ks.Ebnd)
    print('nsp=', nsp, 'nkp=', nkp, 'band_max=', band_max)
    print('len(klist)=', len(ks.klist))
    ks.PRINT = False
...
if __name__ == '__main__':
    ...
    if mrank==master:
        Compute_QGT() # ([(0,4),(4,9)]) # ([(0,9),(9,12)])# ([(16,17)]) # ([(0+15,16+15),(16+15,17+15)]) #[(0+15,16+15),(16+15,17+15),(17+15,20+15)]) #([(11,11+9),(11+9,11+12)]) #([(4,7),(7,12)]) #([(0+18,6+18),(6+18,16+18),(16+18,21+18)])
