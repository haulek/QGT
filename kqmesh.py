...
class KQmesh:
    ...
    def __init__(self, nkdivs, k0shift, strc, latgen, fout):
        ...
        for i in range(len(self.weight)):
            ...
            _klist0_ = array((kl*self.LCM/self.ndiv).round(), dtype=int)
            _klist_ = dot(self.k2icartes, _klist0_ )
            #kc = dot(self.k2icartes, kl)/self.ndiv
            kc = _klist_/float(self.LCM)
            kcc = dot(self.k2cartes, kc)
            print('%3d' %(i,), '[%3d,%3d,%3d]' % tuple(iii), '   ', ('[%4.1f,%4.1f,%4.1f]') % tuple(kl), '%5d' % (self.weight[i],),end='',file=fout)
            print('   ', '[%3d,%3d,%3d]'%tuple(_klist_), '[%6.3f,%6.3f,%6.3f]'%tuple(kc), kcc, file=fout)
