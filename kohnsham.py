...
class KohnShamSystem:
    ...
    def VectorFileRead(self, case, strc, latgen, kqm, pw, fout, in1=None, HaveFullBZ=True):
        def Shift_to_1BZ(k):
            " This gives k_{1BZ}-k"
            if k==0:
                return 0
        ...
        if in1 is not None:
            ...
            if nsp==1:
                ...
                sys.exit(0)
            print('Number of k-points read=', len(all_Ek), file=fout)
        ...
        if HaveFullBZ:  # If we have points only on a path, this can not be done
