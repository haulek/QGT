...
class MatrixElements2Band:
    ...
    def calc_minm(self, ik, iq, band_limits, mode, strc, in1, latgen, kqm, ks, radf, pw, pb, core, rest, t_times, fout, PRINT=False):
        ...
        iumklap = array(( kqm.klist[ik]/float(kqm.LCM) - kqm.klist[jk]/float(kqm.LCM) - ql ).round(),dtype=int)
        ...
        if cend > 0:  # at least one core state, hence compute overlap of Kohn-Sham+core orbitals and product basis
            ...
            t2 = timer()
            # Now compute also
            minc = la_matmul( mmat_c, conj(self.barcvm[:self.loctmatsize,:]) )   #@@!!
    ...
    def calc_selfx(self, iq, strc, in1, latgen, kqm, ks, radf, pw, pb, core, kw, fout):
        ...
        for irk in range(nirkp):
            ...
            ik = kqm.k_ind[irk]   # index in all-kpoints, not irreducible
            jk = kqm.kqid[ik,iq]  # index of k-q
            jrk = kqm.kii_ind[jk] # this will need to be the closest point to k-q
            kpts = (ik,jk,iq)
            ...
            for i in range(len(sx)):
                print('dSigx[irk=%3d,ie=%3d]=%16.12f' % (irk, i+ks.ibgw, sx[i]), file=fout)
            ...
            for icg in range(ks.ncg_p):
                ...
                if PartialTetra:
                    for iw,omega in enumerate(fr.omega):
                        sm = sum(termcv * (-2./ankp)*Edif/( omega**2 + Edif**2))
            ...
            if PartialTetra:
                ...
                for iw,omega in enumerate(fr.omega):
                    for ie2 in range(ns2):
                        ...
                        sm = sum(termvv[ie2,:ne1] * (-2./ankp)*Edif[ie2,:ne1]/( omega**2 + Edif[ie2,:ne1]**2))
                        head[iw] -= coef*sm
                    ...
                    sm = sum(termvv[ns2:ne2,:] * (-2./ankp)*Edif[ns2:ne2,:]/( omega**2 + Edif[ns2:ne2,:]**2))
                    head[iw] -= coef*sm
    ...
    def calc_eps(self, iq, head_quantities, strc, in1, latgen, kqm, ks, radf, pw, pb, core, kw, fr, kcw, ddir, dUl, Ul, fout, PartialTetra=True):
        ...
        for ik,irk in enumerate(kqm.kii_ind):
            ...
            if PartialTetra:
                ...
                fvxcn.compute_m_p_m_notetra2(eps,epsw1,kcw[:,:,:,ik],minm2,enk,enq,fr.omega,pm,iq,Noactual,Neactual,coef,ankp)
    ...
    def calc_selfc(self, iq, eps, epsw1, epsw2, head, strc, in1, latgen, kqm, ks, radf, pw, pb, core, kw, fr, ddir, dUl, Ul, fout, save_mwm=True, PRINT=True):
        ##
        ...
        for irk in range(nirkp):
            ...
            if PRINT:
                for ie1 in range(ks.nbgw-ks.ibgw):
                    #for iom in range(len(fr.omega)):
                    iom=0
                    print('dSigc[iq=%3d,irk=%3d,ie=%3d,iom=%3d]=%16.12f%16.12f' % (iq,irk,ie1+ks.ibgw,iom,sc_p[irk,ie1,iom].real,sc_p[irk,ie1,iom].imag), file=fout)
